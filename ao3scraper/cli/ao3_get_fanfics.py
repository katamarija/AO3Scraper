...
def get_tag_info(category, meta):
    ...
    """
    try:
        tag_list = meta.find("dd", class_=str(category) + " tags").find_all(
            class_="tag"
        )
...
def get_kudos(meta):
    if meta:
        ...
        for kudo in kudos:
            if kudo.name == "a":
                if (
                    "more users" not in kudo.contents[0]
                    and "(collapse)" not in kudo.contents[0]
                ):
...
def get_bookmarks(url, header_info):
    ...
    if soup.find("ol", class_="pagination actions"):
        pages = soup.find("ol", class_="pagination actions").findChildren(
            "li", recursive=False
        )
    ...
    if access_denied(soup):
        ...
        if lang != False and lang != stats[0]:
            ...
            if include_bookmarks:
                bookmark_url = (
                    "http://archiveofourown.org/works/" + str(fic_id) + "/bookmarks"
                )
...
def get_args():
    parser = argparse.ArgumentParser(
        description="Scrape and save some fanfic, given their AO3 IDs."
    )
    parser.add_argument(
        "ids",
        metavar="IDS",
    ...
    parser.add_argument("--csv", default="fanfics.csv", help="csv output file name")
    parser.add_argument("--header", default="", help="user http header")
    parser.add_argument(
        "--restart", default="", help="work_id to start at from within a csv"
    )
    parser.add_argument(
        "--firstchap",
        default="",
    ...
    parser.add_argument("--bookmarks", action="store_true", help="retrieve bookmarks; ")
